-- Gunbound Arena: phao binh luot cho Nokia dial-key (LuaS30 / MTK MRE).
-- Man hinh: splash -> menu -> (play1 | play2 | lang | guide | settings | about) -> over.
local E = engine
...
local function go(name)
    if name == "play1" or name == "play2" then
        screen = "play"
        Game.start(name == "play1" and 1 or 2)
    else
        screen = "ui"
        UI.enter(name)
    end
