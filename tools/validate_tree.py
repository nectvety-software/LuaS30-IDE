...
required=[
  ...
  "vendor/lua-5.1.5/src/lua.h","vendor/lua-5.1.5/src/lapi.c",
]
for rel in required:
    if not (ROOT/rel).is_file(): errors.append("missing: "+rel)
# Installed packages ship bytecode-only tools (build.pyc).
if not ((ROOT/"tools"/"build.py").is_file() or (ROOT/"tools"/"build.pyc").is_file()):
    errors.append("missing: tools/build.py or tools/build.pyc")
