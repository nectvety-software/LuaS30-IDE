...
# Vùng đọc được của scope="engine": lõi MRE của chính IDE, nằm ngoài mọi dự án.
# Tool "engine" RIÊNG đã bị bỏ — read/grep/glob thường trực tiếp nhận thêm
# args.scope="engine" vì hai bộ tool cùng đọc tệp chỉ khác gốc là trùng lặp.
...
class AIReadOnlyToolService:
    ...
    def execute(self, project_root: Path | None, action: ToolAction) -> str:
        ...
        if str(args.get("scope") or "").strip().lower() in {"engine", "ide"}:
            return self._engine_scoped(tool, args)
    ...
    def _engine_root(self) -> Path:
        if not self.engine_root or not self.engine_root.is_dir():
            raise ValueError("Engine root is unavailable for scope=engine.")
    ...
    def _engine_path(self, raw: str) -> Path:
        ...
        if prefix not in ENGINE_ALLOWED_PREFIXES:
            raise ValueError(
                "scope=engine chỉ đọc được: " + ", ".join(ENGINE_ALLOWED_PREFIXES)
            )
        return rel

    def _engine_scoped(self, tool: str, args: dict) -> str:
        """read/grep/glob với gốc là thư mục cài IDE (thay tool `engine` cũ)."""
        root = self._engine_root()
        if tool == "read":
        ...
        if tool == "glob":
            ...
            bases = [self._engine_path(scope)] if scope else self._engine_scopes()
            matches: list[str] = []
            for base in bases:
                for path in base.rglob("*"):
                    ...
                    if self._matches(rel, pattern) or self._matches(rel, f"**/{pattern}"):
                        matches.append(rel)
            matches.sort()
            return self._limit("GLOB " + pattern + " (engine)\n" + ("\n".join(matches) or "(no matches)"))
        if tool == "grep":
        ...
        raise ValueError(f"Unsupported engine-scoped tool: {tool}")
